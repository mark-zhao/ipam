...
func (e *etcd) DeletePrefix(ctx context.Context, prefix Prefix, idc bool) (Prefix, error) {
	if idc {
		if prefs, err := e.ReadAllPrefixes(ctx); err == nil {
			e.lock.Lock()
			defer e.lock.Unlock()
			for _, v := range prefs {
				if v.IDC == prefix.IDC {
					ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
					_, err := e.etcdDB.Delete(ctx, v.Cidr)
					cancel()
					if err != nil {
						return Prefix{}, err
					}
