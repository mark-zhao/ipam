...
func (r *redis) DeletePrefix(ctx context.Context, prefix Prefix, idc bool) (Prefix, error) {
	if idc {
		if prefs, err := r.ReadAllPrefixes(ctx); err == nil {
			r.lock.Lock()
			defer r.lock.Unlock()
			for _, v := range prefs {
				if v.IDC == prefix.IDC {
					_, err := r.rdb.Del(ctx, v.Cidr).Result()
					if err != nil {
						return Prefix{}, err
					}
				}
